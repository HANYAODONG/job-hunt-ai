...
from job_update.government_job_update.core.config import DEFAULT_DATABASE, DEFAULT_SKILL_POOL
from job_update.government_job_update.core.database import GovernmentSQLiteStore
from job_update.government_job_update.core.review_queue import (
...
from job_update.government_job_update.core.cli import _build_update_system
from job_update.company_job_update.core.models import JobPosting
from job_update.company_job_update.core.review_queue import create_pending_reviews, serialize_process_result
from job_update.shared.text_utils import clean_text
...
def preview_one(payload: dict[str, Any]) -> dict[str, Any]:
    """Run the government JD pipeline without writing formal state."""
    ...
    posting = JobPosting(
        ...
        metadata={
            "source": clean_text(payload.get("source")) or "government_web",
            "source_name": clean_text(payload.get("source_name")),
            "publish_time": clean_text(payload.get("publish_time")),
            "recruitment_year": clean_text(payload.get("recruitment_year")),
            "source_url": clean_text(payload.get("source_url")),
            "government_agency": clean_text(payload.get("government_agency")),
            "government_department": clean_text(payload.get("government_department")),
    ...
    system = _build_update_system(_build_args(), store)
    result = system.process(posting, write=False, collect_skills_for_review=True)
    preview_id = str(uuid4())
    _PREVIEWS[preview_id] = {**payload, "job_id": posting.job_id}
    return {
        "preview_id": preview_id,
        "status": "preview",
        "result": serialize_process_result(result, skill_pool_path=DEFAULT_SKILL_POOL),
    }


def submit_preview(preview_id: str, processing_mode: str = "auto") -> dict[str, Any]:
    payload = _PREVIEWS.pop(preview_id, None)
    if payload is None:
        raise KeyError(f"Preview not found or expired: {preview_id}")
...
def submit_one_dry_run(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    posting = JobPosting(
        ...
        job_responsibility=clean_text(payload.get("responsibility")),
        job_requirement=clean_text(payload.get("requirement")), metadata=metadata,
    )
    if not posting.job_title or not posting.month:
        raise ValueError("month and job_title are required")
    system = _build_update_system(_build_args(), store)
