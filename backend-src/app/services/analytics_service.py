...
from .paths import (
    ...
    DATASET_ROOT,
    GOVERNMENT_BASE_TITLE_DICTIONARY,
    GOVERNMENT_JOB_UPDATE_ROOT,
...
def normalize_optimization_skill(skill: str, domain: str = "company") -> dict[str, Any]:
    ...
    if str(normalized_row.get("normalization_status")) == "unresolved":
        try:
            from job_update.company_job_update.skill_extract import extract_job_skills_api as extract_api
            from job_update.company_job_update.skill_extract.normalizer import DEFAULT_CACHE

            cache_path = DEFAULT_CACHE
            prompt_appendix = ""
            if domain == "government":
                from job_update.government_job_update.core.config import DEFAULT_SKILL_NORMALIZATION_CACHE
                from job_update.government_job_update.core.skill_extraction import GOVERNMENT_NORMALIZATION_PROMPT_APPENDIX
...
def _skill_extract_normalizer(domain: str = "company"):
    from job_update.company_job_update.skill_extract.normalizer import SkillNormalizer
    if resolve_domain(domain) == "government":
        from job_update.government_job_update.core.config import DEFAULT_SKILL_EXTRACTION_DICTIONARY
...
def optimization_sources(
    ...
    domain = resolve_domain(domain)
    if domain == "government":
        government_skill_dictionary = (
            GOVERNMENT_JOB_UPDATE_ROOT / "skill_extract" / "government_skill_dictionary.csv"
        )
        government_job_dictionary = GOVERNMENT_BASE_TITLE_DICTIONARY
        configs = [
            {
                "key": "skill_alias",
                "name": "government_skill_dictionary.csv",
                "path": government_skill_dictionary,
                "purpose": "Government skill extraction and normalization dictionary.",
                "required_columns": ["skill_keyword", "normalized_skill", "kg_display_skill"],
            },
            {
                "key": "runtime_job_dictionary",
                "name": "government standard job dictionary",
                "path": government_job_dictionary,
                "purpose": "Government standard jobs, categories, and routing keywords.",
        ...
        return {
            "domain": domain,
            "principle": "Maintain government source dictionaries; derived event and analytics tables are read-only outputs.",
