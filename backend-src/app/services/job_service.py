...
from job_update.company_job_update.core.review_queue import (
...
def preview_one(payload: dict[str, Any]) -> dict[str, Any]:
    """Run the company JD pipeline without writing formal state."""
    progress: list[str] = []
    ...
    posting = _posting_from_payload(payload, source="jd_update_preview")
    result = _build_system(progress).process(
        posting,
        write=False,
    ...
    serialized = serialize_review_process_result(result, skill_pool_path=BASE_SKILL_POOL)
    serialized["progress"] = progress
    return {"preview_id": preview_id, "status": "preview", "result": serialized}
